...
last_modified: '2026-03-24T03:46:33.796Z'
---
...
## Encryption and handshake [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#encryption-and-handshake "Direct link to Encryption and handshake")
...
### Alternative name [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#alternative-name "Direct link to Alternative name")

Given an Ed25519 public key kkk (an octet sequence of length 32), the alternative name N(k)N(k)N(k)
should be computed as follows:

B(n,l)≡{\[\]when l=0\[$abcdefghijklmnopqrstuvwxyz234567\[nmod32\]\]⌢B(⌊n/32⌋,l−1)otherwiseN(k)≡$e⌢B(E32−1(k),52)\\begin{align}
...
\\end{align}
B(n,l)N(k)​≡{\[\]\[$abcdefghijklmnopqrstuvwxyz234567\[nmod32\]\]⌢B(⌊n/32⌋,l−1)​when l=0otherwise​≡$e⌢B(E32−1​(k),52)​​

Where E32−1\\mathcal{E}\_{32}^{-1}E32−1​ is the deserialization function for 256-bit unsigned integers,
defined in the serialization codec appendix of the GP.

### ALPN [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#alpn "Direct link to ALPN")
...
## Required connectivity [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#required-connectivity "Direct link to Required connectivity")
...
based on the Ed25519 public keys of the two validators (ke,k∈Kk\_e, k \\in \\mathbb{K}ke​,k∈K). After a
reasonable timeout (e.g. 5 seconds), the other validator may choose to initiate the connection.
Given two Ed25519 public keys (a,b)(a, b)(a,b), the Preferred Initiator P(a,b)∈{a,b}P(a, b) \\in \\{a, b\\}P(a,b)∈{a,b} is given as:

P(a,b)≡{awhen (a31>127)⊕(b31>127)⊕(a<b)botherwiseP(a, b) \\equiv \\begin{cases}
a &\\text{when } (a\_{31} > 127) \\oplus (b\_{31} > 127) \\oplus (a < b)\\\
b &\\text{otherwise}
\\end{cases}
P(a,b)≡{ab​when (a31​>127)⊕(b31​>127)⊕(a<b)otherwise​
...
### Epoch transitions [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#epoch-transitions "Direct link to Epoch transitions")
...
- At least max(⌊E/30⌋,1)\\text{max}(\\left\\lfloor E / 30 \\right\\rfloor, 1)max(⌊E/30⌋,1) slots have elapsed since the
beginning of the epoch (where EEE is the number of slots in an epoch).
...
## Protocols and streams [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#protocols-and-streams "Direct link to Protocols and streams")
...
### Messages [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#messages "Direct link to Messages")
...
### Notation [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#notation "Direct link to Notation")
...
### Common types [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#common-types "Direct link to Common types")
...
### Grid structure [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#grid-structure "Direct link to Grid structure")
...
### UP 0: Block announcement [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#up-0-block-announcement "Direct link to UP 0: Block announcement")
...
### CE 128: Block request [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-128-block-request "Direct link to CE 128: Block request")
...
### CE 129: State request [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-129-state-request "Direct link to CE 129: State request")
...
### CE 131/132: Safrole ticket distribution [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-131132-safrole-ticket-distribution "Direct link to CE 131/132: Safrole ticket distribution")
...
The first step should be performed max(⌊E/60⌋,1)\\text{max}(\\left\\lfloor E / 60 \\right\\rfloor, 1)max(⌊E/60⌋,1) slots after
the connectivity changes for a new epoch are applied (where EEE is the number of slots in an
...
current validators. Forwarding should be delayed until max(⌊E/20⌋,1)\\text{max}(\\left\\lfloor E / 20
\\right\\rfloor, 1)max(⌊E/20⌋,1) slots after the application of connectivity changes, to avoid exposing the
...
### CE 133: Work-package submission [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-133-work-package-submission "Direct link to CE 133: Work-package submission")
...
### CE 146: Work-package bundle submission [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-146-work-package-bundle-submission "Direct link to CE 146: Work-package bundle submission")
...
### CE 134: Work-package sharing [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-134-work-package-sharing "Direct link to CE 134: Work-package sharing")
...
### CE 135: Work-report distribution [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-135-work-report-distribution "Direct link to CE 135: Work-report distribution")
...
### CE 136: Work-report request [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-136-work-report-request "Direct link to CE 136: Work-report request")
...
### Shard assignment [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#shard-assignment "Direct link to Shard assignment")

Erasure coded shards are assigned to validators as follows:

i=(cR+v)modVi = (cR + v) \\bmod V
i=(cR+v)modV
...
- RRR is the recovery threshold: the minimum number of EC shards required to recover the original
data. With 1023 validators, R=342R = 342R=342. With 6 validators, R=2R = 2R=2.
- VVV is the number of validators.

### CE 137: Shard distribution [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-137-shard-distribution "Direct link to CE 137: Shard distribution")
...
- s\\mathbf{s}s is the sequence of (work-package bundle shard hash, segment shard root) pairs
satisfying u=MB(s)u = \\mathcal{M}\_B(\\mathbf{s})u=MB​(s), where uuu is the erasure-root and MB\\mathcal{M}\_BMB​ is
...
### CE 138: Audit shard request [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-138-audit-shard-request "Direct link to CE 138: Audit shard request")
...
### CE 147: Bundle request [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-147-bundle-request "Direct link to CE 147: Bundle request")
...
### CE 139/140: Segment shard request [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-139140-segment-shard-request "Direct link to CE 139/140: Segment shard request")
...
The number of segment shards requested in a single stream should not exceed 2WM2W\_M2WM​ (WM=3072W\_M = 3072WM​=3072,
...
### CE 148: Segment request [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-148-segment-request "Direct link to CE 148: Segment request")
...
The number of segments requested in a single stream should not exceed WMW\_MWM​ (WM=3072W\_M = 3072WM​=3072,
this constant is defined in the GP).
...
### CE 141: Assurance distribution [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-141-assurance-distribution "Direct link to CE 141: Assurance distribution")
...
### CE 142: Preimage announcement [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-142-preimage-announcement "Direct link to CE 142: Preimage announcement")
...
### CE 143: Preimage request [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-143-preimage-request "Direct link to CE 143: Preimage request")
...
### CE 144: Audit announcement [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-144-audit-announcement "Direct link to CE 144: Audit announcement")
...
### CE 145: Judgment publication [​](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec\#ce-145-judgment-publication "Direct link to CE 145: Judgment publication")
...
- [Encryption and handshake](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#encryption-and-handshake)
  - [Alternative name](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#alternative-name)
  - [ALPN](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#alpn)
- [Required connectivity](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#required-connectivity)
  - [Epoch transitions](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#epoch-transitions)
- [Protocols and streams](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#protocols-and-streams)
  - [Messages](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#messages)
  - [Notation](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#notation)
  - [Common types](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#common-types)
  - [Grid structure](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#grid-structure)
  - [UP 0: Block announcement](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#up-0-block-announcement)
  - [CE 128: Block request](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-128-block-request)
  - [CE 129: State request](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-129-state-request)
  - [CE 131/132: Safrole ticket distribution](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-131132-safrole-ticket-distribution)
  - [CE 133: Work-package submission](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-133-work-package-submission)
  - [CE 146: Work-package bundle submission](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-146-work-package-bundle-submission)
  - [CE 134: Work-package sharing](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-134-work-package-sharing)
  - [CE 135: Work-report distribution](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-135-work-report-distribution)
  - [CE 136: Work-report request](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-136-work-report-request)
  - [Shard assignment](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#shard-assignment)
  - [CE 137: Shard distribution](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-137-shard-distribution)
  - [CE 138: Audit shard request](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-138-audit-shard-request)
  - [CE 147: Bundle request](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-147-bundle-request)
  - [CE 139/140: Segment shard request](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-139140-segment-shard-request)
  - [CE 148: Segment request](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-148-segment-request)
  - [CE 141: Assurance distribution](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-141-assurance-distribution)
  - [CE 142: Preimage announcement](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-142-preimage-announcement)
  - [CE 143: Preimage request](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-143-preimage-request)
  - [CE 144: Audit announcement](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-144-audit-announcement)
  - [CE 145: Judgment publication](https://docs.jamcha.in/knowledge/advanced/simple-networking/spec#ce-145-judgment-publication)
