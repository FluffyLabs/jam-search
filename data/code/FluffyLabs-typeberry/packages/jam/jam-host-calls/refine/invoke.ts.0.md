...
content_kind: code
url: >-
  https://github.com/FluffyLabs/typeberry/blob/main/packages/jam/jam-host-calls/refine/invoke.ts#L1-L104
...
chunk_index: 0
chunk_total: 2
content_sha: 1adfb52aead3dfc4c77a7b1177a0eae1653a5fa33dac646687f9a01338dd1eac
language: typescript
---
`packages/jam/jam-host-calls/refine/invoke.ts` (lines 1–104)
...
export class Invoke implements HostCallHandler {
  ...
  async execute(_gas: IGasCounter, regs: HostCallRegisters, memory: HostCallMemory): Promise<PvmExecution | undefined> {
    ...
    if (readResult.isError) {
      logger.trace`[${this.currentServiceId}] INVOKE(${machineIndex}) <- PANIC (read)`;
    ...
    if (writeResult.isError) {
      logger.trace`[${this.currentServiceId}] INVOKE(${machineIndex}) <- PANIC (write)`;
      return PvmExecution.Panic;
    }
